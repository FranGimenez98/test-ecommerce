...
const addToCart = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
      ...
      if (!cart) {
        cart = new Cart({ _id: new mongoose.Types.ObjectId(), user: userId, items: [], totalPrice: 0 });
      }
      ...
      const size = product.sizes.find((s: any) => s.size === sizeName && s.quantity >= quantity);
  
      if (!size) {
        throw new Error("No hay suficiente stock del producto");
      }
  
      let itemIndex = cart.items.findIndex((i: any) => i.product._id === productId);
      ...
      cart.totalPrice = cart.items.reduce((acc: number, item: any) => acc + item.price, 0);
